...
import { measureText, ensureFontReady } from './text-layout';
import type { TextMeasurement } from './text-layout';

const ARC_AMOUNT = 0.3;
const TEXT_WORLD_SCALE = 0.008;
...
function computeArc(ox: number, oy: number, maxLineWidthPx: number): number {
  ...
  const textWorldWidth = maxLineWidthPx * TEXT_WORLD_SCALE;
...
// Distribute Pretext's per-segment widths down to grapheme widths so we can
// place each character along the arc with shaping-accurate metrics.
function getSegmentGraphemeWidths(
  ...
  // Pretext exposes segment widths at runtime but not on its public type.
...
// Renders the glyph run onto a 2D canvas. charFraction drives the
// typewriter-style reveal; mouseX/mouseY apply local repulsion when present.
...
export async function createCurvedTextMesh(
  ...
  const sw = layout.canW * TEXT_WORLD_SCALE;
  const sh = layout.canH * TEXT_WORLD_SCALE;
...
// World-space bounding box of a rendered thought, used by the placement
// solver to nudge new thoughts away from existing ones.
export function measureThoughtBounds(text: string): { width: number; height: number } {
  const prepared = prepareWithSegments(text, RENDER_FONT);
  const result = layoutWithLines(prepared, RENDER_MAX_WIDTH, RENDER_LINE_HEIGHT);
  return {
    width: Math.max(...result.lines.map(l => l.width)) * TEXT_WORLD_SCALE,
    height: result.lines.length * RENDER_LINE_HEIGHT * TEXT_WORLD_SCALE,
  };
}
